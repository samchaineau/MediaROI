...
def main():
    ...
    #raw_data = st.file_uploader("Choisir un fichier Excel raw data", type="xlsx")
    #mmm_results_data = st.file_uploader("Choisir un fichier Excel des résultats", type="xlsx")
    ...
    if "simulationResults" in st.session_state.keys():
        ...
        with colBudget:
            grouped_invest = st.session_state.simulationResults.drop(["roi", "Allocation %", "Media Channel"], axis = 1).groupby(["Media Type"]).sum().reset_index()
        ...
        with colRoi:
            ...
            st.plotly_chart(px.bar(grouped_contrib.sort_values("ROI"), x='Media Type', y='ROI', color="Media Type"))
